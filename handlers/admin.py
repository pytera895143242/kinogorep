...
async def redarkt_button(message: types.Message, state: FSMContext):
    ...
    for but in arr3:
        new_but = but.split(' | ')
        button = []
        for b in range(0, len(new_but)):
            button.append(types.InlineKeyboardButton(text=new_but[b].split(" - ")[0], url=new_but[b].split(" - ")[1]))
    ...
    try:
        ...
        await state.update_data(arr3_but=arr3)  # Обновление Сета
...
async def fname_step(call: types.callback_query, state: FSMContext):
    ...
    try:  # Пытаемся добавить кнопки. Если их нету оставляем клаву пустой
        ...
        for but in arr3:
            ...
            for b in range(0, len(new_but)):
                button.append(
                    types.InlineKeyboardButton(text=new_but[b].split(" - ")[0], url=new_but[b].split(" - ")[1]))
